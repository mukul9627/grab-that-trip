...
const FeatureList = ({ data }: FeatureListProps) => {
  // 🔐 SAFETY CHECK
  if (!data) return null;
  ...
  const features = [
    {
      ...
      title: "Duration",
      value: `${data.duration_days ?? 0} Days`,
    },
    {
      id: 2,
    ...
    },
    {
      ...
      title: "Inclusions",
      value: `${data.inclusions?.length ?? 0} Items`,
    },
    {
      id: 4,
  ...
  return (
    <ul className="tg-tour-details-video-feature-list">
      {features.map((item) => (
        <li key={item.id}>
          <span className="icon">{item.icon}</span>
          <div>
            <span className="title">{item.title}</span>
            <span className="duration">{item.value}</span>
          </div>
        </li>
      ))}
    </ul>
